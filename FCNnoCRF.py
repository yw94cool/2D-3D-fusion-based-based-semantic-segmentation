...
def inference(image, keep_prob):
    ...
    mean_pixel = [86.59, 92.48, 86.04, 37.69]
...
def main(argv=None):
    ...
    sess = tf.Session(config=tf.ConfigProto(log_device_placement=True, device_count = {'GPU': 0}))#gpu_options = gpu_options
    ...
    if FLAGS.mode == "train":
        ...
        for itr in xrange(MAX_ITERATION):
            ...
            if itr % 50 == 0:
                ...
                if valid_acc > 0.9 or itr == 0:
                    saver.save(sess, "F:/model.ckpt", itr)#DLLQ/FCN.tensorflow-master2.0/FCN.tensorflow-master/logs/

                valid_record.loc[valid_row] = [itr, valid_loss, valid_acc]
                valid_row += 1

            train_record.to_csv("..\\loss_and_acc\\train_record.csv", index=False, sep=',', mode='w')
            valid_record.to_csv("..\\loss_and_acc\\valid_record.csv", index=False, sep=',', mode='w')
    ...
    # 更新：测试
        ...
        test_dir = '..\\test_image\\*crop0[0-9].npy'
        test_file_list = glob.glob(test_dir)
        for f in test_file_list:
            test_image_name = test_dir.split('\\')[-1] #'top_potsdam_7_11_crop01'
            test_path = f #os.path.join(test_dir,  test_image_name + '.jpg')
            # image_test = misc.imread(test_path)
            image_test = np.load (test_path)
            test_image = image_test.reshape((1, image_test.shape[0], image_test.shape[1], image_test.shape[2]))

            start_time = time.time()
            pred_image = sess.run(pred_annotation, feed_dict={image: test_image, keep_probability: 1.0})
            end_time = time.time()
